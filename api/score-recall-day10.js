...
async function scoreSheet(target, accessToken, anthropicKey, budget) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    // If the human Score col P (index 15) is empty, fill it with the AI total.
    // If the teacher has already typed something there, leave it alone.
    const existingScore = (row[15] === undefined ? '' : row[15]).toString().trim();
  ...
  for (const u of updates) {
    await writeRange(target.sheetId, accessToken, `${target.tab}!P${u.rowNum}:AB${u.rowNum}`, [[u.scoreColValue, ...u.aiValues]]);
  }
