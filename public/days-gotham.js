...
const GOTHAM_DAY_14 = {
  ...
  "activities": [
    ...
    },
    {
      ...
      "mins": 60,
      "block": "Match-Build-Make",
      "description": "The ball is off to the side. Use the distance formula for the diagonal to it, and the angle to turn (45° if the ball is equally across and up, otherwise tan⁻¹(across ÷ up)). Turn, drive that many blocks, turn back — then the bot kicks. Most goals out of 5 wins.",
      "facilitatorDescription": "New math: the distance formula AND the launch angle (this is Strategy 3, the general solution). The ball sits at (across, up); teams compute the diagonal c = sqrt(across^2 + up^2), turn it into BLOCKS (c ÷ 27, since runBlockCnt drives 27 cm per block), and find the angle = tan^-1(across ÷ up) — which is just 45 degrees when across = up (half a square, no calculator) and 0 degrees when across = 0 (straight shot). The launch is three moves: turn the angle, runBlockCnt(blocks), turn back the same angle; students paste it into STUDENT ZONE B of the match code, then the locked part drives the last bit and kicks (Power squared). Draw out the big idea: the straight shot and the 45 launch are the SAME rule with special numbers. Set the ball on a Pythagorean triple (60-80, 30-40, 90-120) for clean diagonals. TEST THE KICK yourself first — wide goal, short distance behind the ball — and only compete once bots reliably score: most goals out of 5, one neutral ref. Print the worksheet; open the code page.",
